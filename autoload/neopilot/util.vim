...
function neopilot#util#UTF8Width(str) abort
  return strchars(substitute(a:str, "\\%#=2[^\u0001-\uffff]", "    ", 'g'))
endfunction

function! neopilot#util#PositionToOffset(row, col) abort
  ...
  let text_pre = join(lines_pre, neopilot#util#LineEndingChars())
...
function! neopilot#util#OffsetToPosition(offset) abort  if a:offset < 0
  ...
  let line_ending_len = len(neopilot#util#LineEndingChars())
  let char_offset = a:offset + 1
  let row = 1

  for line in getline(1, '$')
    let line_len = neopilot#util#UTF8Width(line) + line_ending_len
    if line_len >= char_offset
      let col_num = 1

      for s:char in split(getline(row), '\zs')
        let char_width = neopilot#util#UTF8Width(s:char)
