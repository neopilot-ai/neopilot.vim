local enums = require("neopilot.enums")
...
local function neopilot_to_cmp(comp, offset, right)
	...
	local range = {
		start = {
			-- Neopilot returns an empty row for the first line
	...
	return {
		...
		cmp = {
			kind_text = "Neopilot",
			kind_hl_group = "CmpItemKindNeopilot",
		},
		neopilot_completion_id = comp.completion.completionId,
	}
end
...
if imported_cmp then
	cmp.event:on("confirm_done", function(event)
		if
			...
			and event.entry.source.name == "neopilot"
			and event.entry.completion_item
			and event.entry.completion_item.neopilot_completion_id
			and event.entry.source.source
			and event.entry.source.source.server
		then
			event.entry.source.source.server:accept_completion(event.entry.completion_item.neopilot_completion_id)
		end
	end)
end
...
function Source:complete(params, callback)
	...
	local function handle_completions(completion_items)
		...
		for _, comp in ipairs(completion_items) do
			if not duplicates[comp.completion.text] then
				...
				table.insert(completions, neopilot_to_cmp(comp, offset, after_line))
			end
		end
	...
	self.server:request_completion(
		...
		function(success, json)
			...
			if json and json.state and json.state.state == "NEOPILOT_STATE_SUCCESS" and json.completionItems then
